...
self.addEventListener('fetch', function(e){
  ...
  e.respondWith(caches.match(e.request).then(function(cached){return cached||fetch(e.request).then(function(resp){if(resp.ok&&(url.origin===location.origin||url.hostname.indexOf('gstatic')>=0||url.hostname.indexOf('googleapis')>=0||url.hostname.indexOf('cdnjs')>=0)){var copy=resp.clone();caches.open(CACHE).then(function(c){c.put(e.request,copy);});}return resp;});}));
});
